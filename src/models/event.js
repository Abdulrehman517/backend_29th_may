...
const Events = {
        ...
        try {
            ...
            for (let i in data['performers']) {
                ...
                if (performers && performers.performer != null) {
                    ...
                    const values = [performers['performer'], results.insertId, set_rate, performers['paid_status'], data['user_id']];
                    await pool.query(sql, values);
                }
            }
            return results;
    ...
    updateeventDetails: async (id, data) => {
        try {
            ...
            let oldData = await pool.query('SELECT event_name, date, start_event_time, end_event_time, venue_id FROM events WHERE id = ?', [
                id,
            ]);
            const newData = {
            ...
            if (oldEventBriteData.length > 0) {
                ...
                if (diffEventBriteData && diffEventBriteData.length > 0) {
                    ...
                    const activity_values = [
                        ...
                        data.user_id,
                        'event_brite_details',
                        oldEventBriteData[0].id,
                        id,
                        current_utc_date_string2,
                    ];
                    const [activityResults] = await pool.query(sql, activity_values);
                    const activityId = activityResults.insertId;
                    if (activityId) {
                        for (var d in diffEventBriteData) {
                            const sql_change_log = `INSERT INTO change_logs(activity_id, field_name, old_value, new_value)VALUES(?, ?, ?, ?)`;
                            const change_log_values = [
                                activityId,
                                diffEventBriteData[d].field_name,
                                diffEventBriteData[d].old_value,
                                diffEventBriteData[d].new_value,
                            ];
                            await pool.query(sql_change_log, change_log_values);
                        }
                    }
                        ...
                        if (diffPerformers && diffPerformers.length > 0) {
                            ...
                            if (activityId) {
                                for (let d in diffPerformers) {
                                    ...
                                    const change_log_values = [
                                        activityId,
                                        diffPerformers[d].field_name,
                                        diffPerformers[d].old_value,
                                        diffPerformers[d].new_value,
                                    ];
                                    await pool.query(sql_change_log, change_log_values);
                                }
                            }
            ...
            for (let i in data['performers']) {
                ...
                if (performers && performers.performer != null) {
                    ...
                    if (existingRow.length > 0) {
                        ...
                        const insertValues = [performers.performer, id, total_earned, performers.paid_status, data.user_id];
            ...
            if (incomingPerformerIds) {
                ...
                if (deletePerformerIds.length > 0) {
                    ...
                    deletePerformerIds.map(async (deleteValues) => {
                        console.log('if delete values', deleteValues);
                        const sql = `INSERT INTO activities(action, actor, table_name, record_id, event_id,date_time)VALUES(?, ?, ?, ?, ?, ?)`;
                        const activity_values = ['delete', data.user_id, 'event_performers', deleteValues, id, current_utc_date_string2];
                        await pool.query(sql, activity_values);
                    });
                }
            } else {
                await pool.query('DELETE FROM event_performers WHERE event_id = ?', [id]);
                existingPerformerIds.map(async (deleteValues) => {
                    console.log('else delete all values', deleteValues);
                    const sql = `INSERT INTO activities(action, actor, table_name, record_id, event_id,date_time)VALUES(?, ?, ?, ?, ?, ?)`;
                    const activity_values = ['delete', data.user_id, 'event_performers', deleteValues, id, current_utc_date_string2];
                    await pool.query(sql, activity_values);
                });
                        ...
                        if (diffEntryTeam && diffEntryTeam.length > 0) {
                            ...
                            if (activityId) {
                                for (let d in diffEntryTeam) {
                                    ...
                                    const change_log_values = [
                                        activityId,
                                        diffEntryTeam[d].field_name,
                                        diffEntryTeam[d].old_value,
                                        diffEntryTeam[d].new_value,
                                    ];
                                    await pool.query(sql_change_log, change_log_values);
                                }
                            }
            ...
            if (incomingEntryTeamsIds) {
                ...
                if (deleteEntryTeamIds.length > 0) {
                    ...
                    await pool.query(deleteSql, deleteValues);
                    deleteEntryTeamIds.map(async (deleteValues) => {
                        console.log('if delete values entry teams', deleteValues);
                        const sql = `INSERT INTO activities(action, actor, table_name, record_id, event_id,date_time)VALUES(?, ?, ?, ?, ?, ?)`;
                        const activity_values = ['delete', data.user_id, 'event_entry_teams', deleteValues, id, current_utc_date_string2];
                        await pool.query(sql, activity_values);
                    });
                }
            } else {
                await pool.query('DELETE FROM event_entry_teams WHERE event_id = ?', [id]);
                existingEntryTeamIds.map(async (deleteValues) => {
                    console.log('else delete all values entry team', deleteValues);
                    const sql = `INSERT INTO activities(action, actor, table_name, record_id, event_id,date_time)VALUES(?, ?, ?, ?, ?, ?)`;
                    const activity_values = ['delete', data.user_id, 'event_entry_teams', deleteValues, id, current_utc_date_string2];
                    await pool.query(sql, activity_values);
                });
                        ...
                        if (diffPromoter && diffPromoter.length > 0) {
                            ...
                            if (activityId) {
                                for (let d in diffPromoter) {
                                    ...
                                    const change_log_values = [
                                        activityId,
                                        diffPromoter[d].field_name,
                                        diffPromoter[d].old_value,
                                        diffPromoter[d].new_value,
                                    ];
                                    await pool.query(sql_change_log, change_log_values);
                                }
                            }
            ...
            if (incomingPromoterIds) {
                ...
                if (deletePromoterIds.length > 0) {
                    ...
                    await pool.query(deleteSql, deleteValues);
                    deletePromoterIds.map(async (deleteValues) => {
                        console.log('if delete values promoters ids', deleteValues);
                        const sql = `INSERT INTO activities(action, actor, table_name, record_id, event_id,date_time)VALUES(?, ?, ?, ?, ?, ?)`;
                        const activity_values = [
                            'delete',
                            data.user_id,
                ...
                existingPromoterIds.map(async (deleteValues) => {
                    console.log('else delete all values promoters', deleteValues);
                    const sql = `INSERT INTO activities(action, actor, table_name, record_id, event_id,date_time)VALUES(?, ?, ?, ?, ?, ?)`;
                    const activity_values = ['delete', data.user_id, 'promoter_guests_history', deleteValues, id, current_utc_date_string2];
                    await pool.query(sql, activity_values);
                });
            }
